...
# Initialize FastAPI and Musixmatch API
app = FastAPI(title="Floatify Musixmatch API")
api = MusixMatchAPI()
...
@app.get("/")
def root():
    return {"message": "Floatify Musixmatch API is running"}


@app.get("/search")
def search_tracks(q: str = Query(..., description="Search query for tracks")):
    """Search for tracks by query string"""
    return api.search_tracks(q)


@app.get("/lyrics")
def get_lyrics(
    track_id: str = Query(None, description="Musixmatch track ID"),
    track_isrc: str = Query(None, description="ISRC code of the track")
):
    """Get lyrics for a track by track_id or ISRC"""
    return api.get_track_lyrics(track_id=track_id, track_isrc=track_isrc)
...
def get_track(
    ...
    """Get track details by track_id or ISRC"""
    return api.get_track(track_id=track_id, track_isrc=track_isrc)
...
def get_richsync(
    track_id: str = Query(None, description="Musixmatch track ID"),
    track_isrc: str = Query(None, description="ISRC code of the track"),
    commontrack_id: str = Query(None, description="Musixmatch commontrack ID")
):
    """Get synced lyrics (richsync) for a track"""
    return api.get_track_richsync(
        track_id=track_id,
